...
class ProductController extends Controller
{
        ...
        $type = $request->type;
         ...
         return Datatables::of($datas)
                            ...
                            ->addColumn('status', function(Product $data) {
                             ...
                             ->addColumn('admin_approved', function(Product $data) {
                                 $class = $data->admin_approved == 1 ? 'drop-success' : 'drop-danger';
                                    if ($data->admin_approved==1){
                                        $data->admin_approved='Approved';
                                    }
                                    else if($data->admin_approved==0){
                                        $data->admin_approved='Pending';
                                    }
                                    else
                                    {
                                        $data->admin_approved='Rejected';
                                    }
                                 return
                                            $data->admin_approved;
    ...
    {
        $type = 'index';
        return view('vendor.product.index',compact('type'));
    }
...
//                $input['childcategory_id'] = $request->childcategory_id;
    ...
    public function update(Request $request, $id)
    {
        ...
        $input = $request->all();
        $input['admin_approved'] =0;
        $input['admin_remarks'] ='Pending from Admin.';
        if ($file = $request->file('photo'))
        {
            $name = time().$file->getClientOriginalName();
            $file->move('assets/images/products',$name);
