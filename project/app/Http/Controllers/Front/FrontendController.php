...
// CURRENCY SECTION ENDS
    ...
    {
        HelperFunctions::writeLogInfo('slug: '.$slug);
        if(mb_strlen($slug,'utf-8') > 1){
            $search = ' '.$slug;
            $prods = Product::where('name', 'like', '%' . $slug . '%')
                ->where('status','=',1)
                ->where('admin_approved','=',1)
               // ->orWhere('name', 'like', $slug . '%')
                ->orWhere('part_no', 'like', $slug . '%')
                ->take(10)->get();
           // dd($prods);
            HelperFunctions::writeLogInfo('products : '.json_encode($prods));
