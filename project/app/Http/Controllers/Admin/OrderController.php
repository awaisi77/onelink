...
class OrderController extends Controller
{
    ...
    public function datatables(Request $request, $status)
    {
        ...
        if (!empty($request->from_date)) {
            if ($request->status != 'all') {
                $datas = Order::whereBetween('created_at', array($request->from_date, $request->to_date))
            ...
            } elseif ($status == 'processing'|| $request->status=='processing') {
                $datas = Order::where('status', '=', 'processing')->orderBy('id', 'desc')->get();
            } elseif ($status == 'completed' || $request->status=='completed') {
                $datas = Order::where('status', '=', 'completed')->orderBy('id', 'desc')->get();
            } elseif ($status == 'declined' || $request->status=='declined') {
                $datas = Order::where('status', '=', 'declined')->orderBy('id', 'desc')->get();
            } elseif ($status == 'on delivery' || $request->status=='on delivery') {
                $datas = Order::where('status', '=', 'on delivery')->orderBy('id', 'desc')->get();
            }
            elseif ($status == 'withheld' || $request->status=='withheld') {
                $datas = Order::where('status', '=', 'withheld')->orderBy('id', 'desc')->get();
            }
            else {
    ...
    public function update(Request $request, $id)
    {
        ...
        if ($data->status == "completed") {
            ...
            if ($input['status'] == "withheld") {
                $gs = Generalsetting::findOrFail(1);

                $track_notes = !empty($request->track_text)?$request->track_text:'';

                if ($gs->is_smtp == 1) {
                    $maildata = [
                        'to' => $data->customer_email,
                        'subject' => 'Your order ' . $data->order_number . ' is Withheld by admin!',
                        'body' => "Hello " . $data->customer_name . "," . "\n We are sorry for the inconvenience caused. Please contact admin for details. ".$track_notes,
                    ];
                    $mailer = new ActiveHouseMailer();
                    $mailer->sendCustomMail($maildata);
                } else {
    ...
    {
        return view('admin.order.withheld');
    }
