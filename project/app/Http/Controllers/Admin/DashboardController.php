...
class DashboardController extends Controller
{
    ...
    {
        HelperFunctions::writeLogInfo("Request : ".json_encode($request->all()));


        //return config('services.ActiveHouse.ocean');
        $purchase_code =  $request->pcode;
        $my_script =  'KingCommerce';
        $my_domain = url('/');

        $varUrl = str_replace (' ', '%20', config('services.ActiveHouse.ocean').'purchase112662activate.php?code='.$purchase_code.'&domain='.$my_domain.'&script='.$my_script);
        $stream_opts = [
            "ssl" => [
                "verify_peer"=>false,
        ...
        if( ini_get('allow_url_fopen') ) {
            $contents = file_get_contents($varUrl , false, stream_context_create($stream_opts));
        }else{
            $ch = curl_init();
        ...
        HelperFunctions::writeLogInfo("result from pc api : ".json_encode($chk));
    ...
    public function movescript(){
        ...
        $files = public_path();
        $bkupname = 'ActiveHouseCart-By-1LinkIndustry-'.date('Y-m-d').'.zip';
