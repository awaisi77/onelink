...
class ProductController extends Controller
{
    ...
    public function approvalDatatable()
    {
        $datas = Product::where('admin_approved', '=', 0)->orderBy('id', 'desc')->get();
        ...
        return Datatables::of($datas)
            ...
            ->addColumn('status', function (Product $data) {
                return '<div class="action-list">
                        <a href="' . route('admin-prod-approval-accept', $data->id) . '" class="view" >Approve</a>
                        <a data-href="' . route('admin-prod-approval-deny', $data->id) . '"
                        data-container="#modalRejectComments"
                        data-toggle="modal"
                        data-target="#modalRejectComments"
                        class="view deny-modal"> Reject</a>
                        </div>';
            })
            ->addColumn('action', function (Product $data) {
                $catalog = $data->type == 'Physical' ? ($data->is_catalog == 1 ? '<a href="javascript:;" data-href="' . route('admin-prod-catalog', ['id1' => $data->id, 'id2' => 0]) . '" data-toggle="modal" data-target="#catalog-modal" class="delete"><i class="fas fa-trash-alt"></i> Remove Catalog</a>' : '<a href="javascript:;" data-href="' . route('admin-prod-catalog', ['id1' => $data->id, 'id2' => 1]) . '" data-toggle="modal" data-target="#catalog-modal"> <i class="fas fa-plus"></i> Add To Catalog</a>') : '';
    ...
    {
        return view('admin.product.approval');
    }
...
// PRODUCT DELETE ENDS
    ...
    public function approveVendorProduct($id)
    {
        ...
        $prod->admin_approved = 1;
        $prod->admin_remarks = 'Approved by Admin.';
        $prod->save();
    ...
    public function denyVendorProductModal($id)
    {

        return view('admin.product.deny_modal', compact('id'));
    }

    public function denyVendorProduct(Request $request, $id)
    {


        $prod = Product::findOrFail($id);
        $prod->admin_remarks = $request['admin_remarks'];
        $prod->admin_approved = 2;
        $prod->save();
