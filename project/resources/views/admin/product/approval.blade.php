...
@section('content')
    ...
    <div class="content-area">
        <div class="mr-breadcrumb">
            <div class="row">
                <div class="col-lg-12">
                    <h4 class="heading">{{ __("Admin Approval Products") }}</h4>
                    <ul class="links">
                        <li>
                        ...
                        <li>
                            <a href="{{ route('admin-prod-approval') }}">{{ __("Admin Approval Products") }}</a>
                        </li>
                    </ul>
                </div>
            </div>
    ...
    {{-- Deny MODAL START --}}
    <div class="modal fade" id="modalRejectComments" tabindex="-1" role="dialog" aria-labelledby="modalRejectComments" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="submit-loader">
                    <img src="{{asset('assets/images/'.$gs->admin_loader)}}" alt="">
                ...
                <div class="modal-header d-block text-center">
                    <h4 class="modal-title d-inline-block">{{ __("Rejection Comments") }}</h4>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
...
@section('scripts')
    ...
    <script type="text/javascript">
        ...
        var table = $('#ActiveHousetable').DataTable({
            ...
            processing: true,
            serverSide: true,
            ajax: '{{ route('admin-prod-approval-datatables') }}',
    ...
    <script type="text/javascript">
        ...
        $(document).on('click','.deny-modal',function(){
            if(admin_loader == 1)
            {
                $('.submit-loader').show();
            }

            $('#modalRejectComments .modal-content .modal-body').html('').load($(this).attr('data-href'),function(response, status, xhr){
                if(status == "success")
                {
                    if(admin_loader == 1)
                    {
