        ...
        @if($user->checkWarning())
            ...
            <div class="alert alert-danger validation text-center">
                <h3>{{ $user->displayWarning() }} </h3> <a
                        href="{{ route('vendor-warning',$user->verifies()->where('admin_warning','=','1')->orderBy('id','desc')->first()->id) }}"> {{$langg->lang803}} </a>
            </div>

        @elseif(count(\App\Models\Verification::where('admin_warning',1)
