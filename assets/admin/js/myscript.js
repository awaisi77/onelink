      ...
      $(document).on('change','#cat',function () {
        ...
        if(link != "")
        {
          ...
          $('#subcat').prop('disabled',false);
            $('#sub_cat_block').css('display','block');
         // $('#sub_cat_block').show();
      ...
      $(document).on('change','#subcat',function () {
        ...
        if(link != "")
        {
          ...
          $('#childcat').prop('disabled',false);
            $('#child_sub_cat_block').css('display','block');
        }
...
$(document).on('submit','#ActiveHouseformdata',function(e){
  e.preventDefault();
if(admin_loader == 1)
  {
  $('.submit-loader').show();
        ...
        if(admin_loader == 1)
          {
            ...
            $('#ActiveHouseformdata input , #ActiveHouseformdata select , #ActiveHouseformdata textarea').eq(1).focus();
          }
          else
          {
           // table.ajax.reload();
...
$(document).on('submit','#ActiveHouseform',function(e){
e.preventDefault();
if(admin_loader == 1)
  {
$('.gocover').show();
...
$('button.addProductSubmit-btn').prop('disabled',true);
    $.ajax({
     ...
     success:function(data)
     {
        console.log(data);
        if ((data.errors)) {
        ActiveHouseform.parent().find('.alert-success').hide();
        ActiveHouseform.parent().find('.alert-danger').show();
        ActiveHouseform.parent().find('.alert-danger ul').html('');
          for(var error in data.errors)
          {
            $('.alert-danger ul').append('<li>'+ data.errors[error] +'</li>')
          }
          ActiveHouseform.find('input , select , textarea').eq(1).focus();
        }
        else
        {
          ActiveHouseform.parent().find('.alert-danger').hide();
          ActiveHouseform.parent().find('.alert-success').show();
          ActiveHouseform.parent().find('.alert-success p').html(data);
          ActiveHouseform.find('input , select , textarea').eq(1).focus();
        }
          if(admin_loader == 1){
        $('.gocover').hide();
